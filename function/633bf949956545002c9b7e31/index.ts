...
const SINGLEPLAY_PAST_MATCHES = "66deee33a6fbf1002b01c522"
...
export async function getUniquePlayerMsisdn() {
    ...
    const userObjIds = userIds.map(el => ObjectId(el))
    const users = await db.collection(`bucket_${USER_BUCKET}`).find({ _id: { $in: userObjIds } }).toArray();
    const usersIdentities = users.map(el => ObjectId(el.identity))
    ...
    db.collection("bucket_67cf0356f7ba00002c33a19e").insertMany(insertArray)
    return "ok"
}
export async function monthlyPlayControl() {
    const db = await database();

    const users = await db.collection("bucket_67cf0356f7ba00002c33a19e")
    ...
    if (bulkOps.length > 0) {
        db.collection("bucket_67cf0356f7ba00002c33a19e").bulkWrite(bulkOps);
    }
