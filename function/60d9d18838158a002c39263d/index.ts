...
export default async function (change) {
    ...
    let template = await Bucket.data
        .getAll(buckets.templates,{
            queryParams: { filter: `template=='${change.document.template}'` }
        })
        .catch(err => console.log("ERROR 42", err));
    template = template[0];
    let content = template.content;
    let variables = JSON.parse(change.document.variables);
    let emails = change.document.emails;
    let reportType = change.document.report_type;
    let html;
    
    if(template.template == "report-pool-mail"){
        html = await getDataPool(reportType).catch(err => console.log("ERROR 99", err));
    } else if (template.template == "report-mail"){
        html = await getData(reportType).catch(err => console.log("ERROR 9", err));
    }
...
//TODO
async function getDataPool(reportType){
    let date = new Date();
    if (reportType == 0) {
    ...
    const chargeHtml = await poolChargeReport(reportType, dateFilter).catch(err =>
        console.log("ERROR 127", err)
    );
    let html = chargeHtml;
    return html;
}
...
//TODO
async function poolChargeReport(reportType, dateFilter){
    ...
    const db = await database();
    const poolChargesCollection = db.collection(`bucket_${POOL_CHARGE_REPORT_BUCKET_ID}`);
    
    let chargeData = await poolChargesCollection.find({
        date: dateFilter})
        .toArray().catch(err => console.log("ERROR: 2", err));
    
    if (defaultReportType == 1 || defaultReportType == 2){
        let currentDate = new Date();
        chargeData.forEach(data =>{
    ...
    chargeData.forEach(data => {
        ...
        <tr>
         <td style="width: 20%;">${new Date(data.date).toLocaleDateString()}</td> 
         <td style="width: 20%;">${numberWithDot(data.game)}</td> 
         <td style="width: 20%;">${numberWithDot(data.channel)}</td> 
         <td style="width: 20%;">${numberWithDot(data.charge_amount)}</td>  
         <td style="width: 20%;">${numberWithDot(data.quantity)}</td> 
         <td style="width: 20%;">${numberWithDot(data.revenue)}TL</td> 
        </tr>`;
    });
    let poolChargesHtml = `
    <h4>Turkcell Hediye Havuzu</h4>
    <table style="width: 100%; border-collapse: collapse;">
        <tbody>
        <tr>
        <th style="width: 20%; text-align:left">Tarih</th>
        <th style="width: 20%; text-align:left">Oyun</th>
        <th style="width: 20%; text-align:left">Kanal</th>
        <th style="width: 20%; text-align:left">Ücret</th>
        <th style="width: 20%; text-align:left">Başarılı Charging</th>
        <th style="width: 20%; text-align:left">Gelir</th>
        </tr>
        ${tableBody}
        </tbody>
    </table>`;
    return poolChargesHtml;
}
