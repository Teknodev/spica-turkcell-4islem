            ...
            if (msisdn) {
                ...
                pastMatches.forEach(data => {
                    ...
                    let obj = {
                        ...
                        winner: data.winner,
                        duel_type: data.duel_type == 0 ? "PVP" : "PVE",
...
export async function dashboardManuallyRewardSecond(req,res){
    if (req.query.key == "wutbztACHHbT") {
        let msisdn = "";
        let begin = new Date("12-31-2021 21:00:0");
        let end = new Date();
        ...
        if (req.query.filter != "undefined"){
            ...
            if(msisdn){
                ...
                manuallyRewards = await manuallyRewardsCollection
                    .find({
                        msisdn: msisdn,
                    ...
                    manuallyRewards = manuallyRewards.map(data => {
                    ...
                    return{
                        ...
                        msisdn: data.msisdn,
                        reward: data.reward,
                        created_at: data.created_at && data.created_at.toLocaleString(),
        ...
        return {
            title: "Manually Rewards",
            data: manuallyRewards,
            displayedColumns: ["_id", "msisdn","reward", "created_at", "process_completed", "system", "retry_id"],
            filters: [
                { key: "msisdn", type: "number", value:msisdn, title:"msisdn"},
                { key: "begin", type: "date", value: begin },
                { key: "end", type: "date", value: end }]
        };
    } else
...
export async function downloadUserAnalysisReport(req, res) {
    ...
    //     if (!msisdn) {
    //         return { error: "Msisdn cannot be empty" }
    //     }
    ...
    //     const pastMatches = await Bucket.data
    //         .getAll(PAST_MATCHES_BUCKET_ID, {
    //             queryParams: {
    //                 filter: {
    //                     $or: [{ user1: userId.toString() }, { user2: userId.toString() }],
    //                     start_time: { $gte: `Date(${beginDate.toISOString()})`, $lte: `Date(${endDate.toISOString()})` }
    //                 },
    //                 sort: { start_time: -1 },
    //                 relation: true
    //             }
    ...
    //         let is_free = data.user1 ? data.user1_is_free : data.user2_is_free
    //         let date = `${startTime.getDate()}/${startTime.getMonth()}/${startTime.getFullYear()} ${getTwentyFourHourTime(startTime.toLocaleTimeString())}`

    //         let obj = {
    //             date: date,
    //             msisdn: msisdn,
    //             services_name: "4 İşlem Bol GB",
    //             fee_type: is_free ? "free" : "paid",
    //             charge_amount: is_free ? "-" : "4TL",
    //             winner: data.winner,
    //             opponent: data.duel_type == 0 ? `${data.user2_is_free ? 'free' : 'paid'}` : 'bot'
    //         };
    //         tableData.push(obj);
    //     });
    ...
    //     res.headers.set(
    //         "Content-Disposition",
    //         `attachment; filename=${msisdn}_report.xlsx`
    //     );
