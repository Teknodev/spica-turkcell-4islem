...
export async function sendSms(receiverMsisdn, code) {
    let message = `Sifreniz: ${code}. Kodu ekrana girerek vergiler dahil ${CHARGE_AMOUNT} karsiliginda 4 Islem Bol GB oyunundan Gunluk 1 GB kazanacaksiniz. Oyunu kazanirsaniz ek olarak Gunluk 1 GB daha kazanacaksiniz. Basarilar!`;
...
export async function sessionSOAP(spUsername, password, serviceVariantId) {
...
export async function singlePlayMatchReward(change) {
    ...
    if (match.user_is_free || match.user_points < 350) {
        return;
    }
    let user = change.document.user;

    const userMsisdn = await getUserDataSinglePlay(user).catch(err => console.log("ERROR 14", err));
    setAwardSinglePlay(userMsisdn, change.document._id);

    return;
}

async function getUserDataSinglePlay(user) {
    if (!db) {
        db = await database().catch(err => console.log("ERROR 22", err));
    }
    Identity.initialize({ apikey: `${SECRET_API_KEY}` });
    ...
    return userIdentity.attributes.msisdn
}
async function setAwardSinglePlay(msisdn, matchId) {
    const sessionId = await sessionSOAP(TCELL_USERNAME, TCELL_PASSWORD, CHARGE_VARIANT).catch(err =>
        console.log("ERROR 15", err)
    );
    if (sessionId) {
